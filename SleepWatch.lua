...
local function chat(message)
    print(string.format('[SleepWatch] %s', message));
end
...
local function status_rows(t)
    ...
    local rows = T{
        header = panel_line('[ SLEEPWATCH ]  Created by Afoofa'),
        info = '',
        timer = '',
        action = '',
    ...
    if (not sw.config.enabled) then
        ...
        rows.action = panel_line('Use /sw on to enable');
...
local function print_help()
    chat('Commands:');
    chat('/sw start [seconds]  - manually start a sleep cycle (default 60)');
    chat('/sw nuke             - manually mark the NUKE call now');
    chat('/sw reset            - clear the current cycle');
    chat('/sw delay <seconds>  - set BLM-start to Sleepga-start delay (default 5.5)');
    chat('/sw duration <secs>  - set the default sleep duration');
    chat('/sw add <name>       - force-track a BLM name; /sw remove <name>');
    chat('/sw blms             - list detected/configured BLMs');
    chat('/sw on | off | help');
end
...
local function list_blms()
    ...
    chat(#found > 0 and ('Tracked: ' .. found:concat(', ')) or 'No BLMs detected. Use /sw add Name if needed.');
end
...
ashita.events.register('load', 'sleepwatch_load', function()
    ...
    settings.register('settings', 'sleepwatch_settings', function(s)
        if (s ~= nil) then
    ...
    chat('SleepWatch v1.1.1 loaded - Created by Afoofa.');
    chat('Sleepga completions start the 60s cycle automatically. /sw help');
end);

ashita.events.register('packet_in', 'sleepwatch_packet_in', function(e)
...
ashita.events.register('command', 'sleepwatch_command', function(e)
    local args = e.command:args();
    if (#args == 0) then
        return;
    end
    local root = normalize_name(args[1]);
    if (root ~= '/sw' and root ~= '/sleepwatch') then
        return;
    end
    e.blocked = true;
...
ashita.events.register('d3d_present', 'sleepwatch_present', function()
...
ashita.events.register('unload', 'sleepwatch_unload', function()
